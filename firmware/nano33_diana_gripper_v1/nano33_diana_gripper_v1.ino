 ...
 */
#define have_hx711   1
#define N_LOADCELLS  (4)
#include "HX711_LITE.h"

HX711_LITE  loadcells[ N_LOADCELLS ];
long        loadcell_data[ N_LOADCELLS ] = { 0L, 0L, 0L, 0L };
long        loadcell_last_read_millis[ N_LOADCELLS ]  = { 0L, 0L, 0L, 0L };
int         loadcell_enabled[ N_LOADCELLS ]    = { 1, 1, 1, 1 };
int         loadcell_errors[ N_LOADCELLS ]     = { 0, 0, 0, 0 };
int         loadcell_clk_pins[ N_LOADCELLS ]  = { 2, 4, 6, 8 };
int         loadcell_data_pins[ N_LOADCELLS ]  = { 3, 5, 7, 9 };
...
bool    have_lf_imu = false;
MPU6050 lf_imu( 0x68 ); // 0x68 for AD0 low, currenty MF (middle-finger)
int16_t     lf_imu_ax, lf_imu_ay, lf_imu_az;
int16_t     lf_imu_gx, lf_imu_gy, lf_imu_gz;

bool    have_rf_imu = false;
MPU6050 rf_imu( 0x69 ); // 0x69 for AD0 high, currently TH (thumb)
int16_t     rf_imu_ax, rf_imu_ay, rf_imu_az;
int16_t     rf_imu_gx, rf_imu_gy, rf_imu_gz;
...
/* 
 * optional Neopixel RGB 12-ring connected to pin 11
 */
#include <Adafruit_NeoPixel.h>

Adafruit_NeoPixel neopixel_ring  = Adafruit_NeoPixel(12, 11, NEO_GRB + NEO_KHZ800);
...
// the diana_gripper has four servos LFJ1 LFJ2 RFJ1 RFJ2
// 
#define N_JOINTS  (4)
#define N_SERVOS  (4)
...
// factory calibration values (degrees -> Hall-effect counts)
// 
#define LFJ1_P00      2048
#define LFJ2_P00      2048
#define RFJ1_P00      2048
#define RFJ2_P00      2048
...
String servo_names[ N_SERVOS ]          = { "LFJ1", "LFJ2",   "RFJ1",  "RFJ2" };
int    servo_ids[ N_SERVOS ]            = {     20,    21,        22,     23  }; 
int    servo_types[N_SERVOS]            = { SMS_SERVO, SMS_SERVO, SMS_SERVO, SMS_SERVO };
int    servo_open_positions[ N_SERVOS ] = {   1024,  1024,      3072,   3072  };
int    servo_zero_positions[ N_SERVOS ] = {   2048,  2048,      2048,   2048  };
int    servo_flex_positions[ N_SERVOS ] = {   3072,  3072,      1024,   1024  };
int    servo_signs[ N_SERVOS ]          = {     +1,    +1,        -1,     -1  }; // +1: increase on close
int    servo_resolution[ N_SERVOS ]     = {   4096,  4096,      4096,   4096  }; // counts per 2pi

int    servo_enabled[ N_SERVOS ]        = {      1,     1,         1,      1  };
int    servo_timeouts[ N_SERVOS ]       = {      0,     0,         0,      0  };
int    servo_torque_limits[ N_SERVOS ]  = {    512,   512,       512,    512  };

       // defaults are medium acceleration and fast speed (range 1..4095 each)
int    servo_accels[ N_SERVOS ]         = {    100,   100,       100,    100  };
int    servo_speeds[ N_SERVOS ]         = {    500,   500,       500,    500  };
...
double joint_radians[N_SERVOS];
double joint_velocities[ N_SERVOS ];
double joint_torques[ N_SERVOS ];
...
void usage() 
{
  ...
  Serial.println();
  Serial.println( "A         <int>*4 or <int>*1 set servo accel(s) (SMS) [0 .. 4095]" );
  Serial.println( "B         <int>*4 or <int>*1 set servo speed(s) (SMS) [0 .. 4095]" );
  Serial.println( "C         <int> close fingers by given degrees" );
  Serial.println( "D         start/stop/continue demo" );
  Serial.println( "E         <int>*4 effort goal (counts)" );
  Serial.println( "F         freeze (at current position)" );
  Serial.println( "G         <int>*4 position goal (counts)" );
  Serial.println( "H         move to home (all zero)" );
  Serial.println( "" );
  Serial.println( "K         <int>*3 set neopixel K_olor (r g b)" );
  Serial.println( "L         <int>*4 set single neopixel L_ed (index r g b)" );
  Serial.println( "M         set mode: 0=idle 1=pos 2=vel 3=teach" );
  Serial.println( "N         <rrggbb>*12 set N_eopixel pattern: N 0 255 65535 16711680..." );
...
{
  sprintf( print_buffer, "%s %d %d %s %d %d %d %d\n",
                 prefix, 
                 (seq_num++) & 0xff, 
                 millis() & 0xff, 
                 mnemonic, 
...
{
  sprintf( print_buffer, "%s %d %d %s %ld %ld %ld %ld \n",
                 prefix, 
                 (seq_num++) & 0xff, 
                 millis() & 0xff, 
                 mnemonic, 
...
void sendServoTemperatures() {
  ...
  if (verbose > 4) {
    sprintf( print_buffer, "... updateServoTUV took %d usec.\n", (t1-t0) );                   
    Serial.print( print_buffer ); Serial.flush();
  }
...
void sendServoCurrents() {
  ...
  printStampedIntArray( "I", "amps", currents );
}
...
void sendAccelGyro() {
  ...
  if (have_lf_imu) { // MPU 6050 at address 0x68
    lf_imu.getMotion6( &lf_imu_ax, &lf_imu_ay, &lf_imu_az,  &lf_imu_gx, &lf_imu_gy, &lf_imu_gz );
    t_last_lf_imu = micros();
    // send data for gyro 1 (FF)
    sprintf( print_buffer, "G %d %d 1 accel %d %d %d gyro %d %d %d \n",
                 (seq_num++) & 0xff, 
                 millis() & 0xff, 
                 lf_imu_ax, lf_imu_ay, lf_imu_az, lf_imu_gx, lf_imu_gy, lf_imu_gz );
    Serial.print( print_buffer ); Serial.flush();
  }

  if (have_rf_imu) { // MPU 6050 at address 0x69
    rf_imu.getMotion6( &rf_imu_ax, &rf_imu_ay, &rf_imu_az,  &rf_imu_gx, &rf_imu_gy, &rf_imu_gz );
    t_last_rf_imu = micros();
    // send data for gyro 1 (FF)
    sprintf( print_buffer, "G %d %d 2 accel %d %d %d gyro %d %d %d \n",
                 (seq_num++) & 0xff, 
                 millis() & 0xff, 
                 rf_imu_ax, rf_imu_ay, rf_imu_az, rf_imu_gx, rf_imu_gy, rf_imu_gz );
    Serial.print( print_buffer ); Serial.flush();
  }
...
void init_MPU6050() 
{
  Serial.println( "... init_MPU6050..." );
  
  Serial.println( "... initializing I2C bus..." );
    #if I2CDEV_IMPLEMENTATION == I2CDEV_ARDUINO_WIRE
        Wire.begin();
        Serial.println( "... using Wire library." );
    #elif I2CDEV_IMPLEMENTATION == I2CDEV_BUILTIN_FASTWIRE
        Fastwire::setup(400, true);
        Serial.println( "... using FastWire library at 400 kHz clock." );
    #endif
  Serial.flush();

  if (!have_lf_imu) {
    Serial.println( "WARNING: init_MPU6050 called, but have_lf_imu is false. Re-initializing..." );
    lf_imu.initialize();
    delay( 500 );
  }

  if (!have_rf_imu) {
    Serial.println( "WARNING: init_MPU6050 called, but have_rf_imu is false. Re-initializing..." );
    rf_imu.initialize();
    delay( 500 );
  }
  ...
  have_rf_imu = false;
    ...
    //Serial.println("... testing MPU6050 device connections...");
    //have_6050 = accelgyro.testConnection();
    //Serial.println(  have_6050 ? "... MPU6050 connection successful" : "### MPU6050 connection failed");
    
    if (have_lf_imu) {
      // optional device initialization & calibration steps
...
void init_HX711() 
{
  ...
  for (int i=0; i < N_LOADCELLS; i++ ) {
    sprintf( print_buffer, "... initializing loadcell %f data pin %d clk pin %d...",
...
void init_neopixel_ring()
{
  Serial.println( "... init_NeopixelRing..." );
  neopixel_ring.begin();
  neopixel_ring.setBrightness( 255 );
  neopixel_ring.show();  
}
...
/**
 * upload the given color to all known LEDs     
 */
void neopixel_single_color( int r,  int g, int b )
{
  for( int i=0; i<neopixel_ring.numPixels(); i++) {
    neopixel_ring.setPixelColor(i, neopixel_ring.Color( r, g, b ));
  }
  neopixel_ring.show(); 
}


void neopixel_one_rgb( int led, int r, int g, int b )
{
  neopixel_ring.setPixelColor( led, neopixel_ring.Color( r, g, b ));
  neopixel_ring.show();
}



void neopixel_rainbow( uint8_t wait ) {
  uint16_t i, j;

  for(j=0; j<256; j++) {
    for(i=0; i<neopixel_ring.numPixels(); i++) {
      neopixel_ring.setPixelColor(i, Wheel((i+j) & 255));
    }
    neopixel_ring.show();
    delay(wait);
  }
...
//
void neopixel_rainbow_cycle( uint8_t wait ) {
  uint16_t i, j;

  for(j=0; j<256*1; j++) { // 1 cycle of all colors on wheel
    for(i=0; i< neopixel_ring.numPixels(); i++) {
      neopixel_ring.setPixelColor(i, Wheel(((i * 256 / neopixel_ring.numPixels()) + j) & 255));
    }
    neopixel_ring.show();
    delay(wait);
  }
  ...
  while( Serial.available()>0 ) {
    ...
    switch( cmd ) {
      ...
      case 'A': // set acceleration limit (SMS). TODO: determine valid range(s)...
                { // first, check whether we have seven tokens
                  int a0, a1, a2, a3;
                  n_tokens = sscanf( serialCommandBuffer.c_str(), "A %d %d %d %d", 
                                     &a0, &a1, &a2, &a3 );
                  if (n_tokens == 4) { // ok, all values are there
                    sprintf( print_buffer, "### Set SMS accelerations %d %d %d %d \n", a0, a1, a2, a3 );
                    Serial.print( print_buffer ); Serial.flush();
                  ...
                  // second attempt: one parameter to be used for all seven servos
                  ...
                  // neither seven nor one parameters: complain
                  sprintf( print_buffer, "### Unknown command '%s'\n", serialCommandBuffer.c_str() );
                  Serial.print( print_buffer ); Serial.flush();
                  removeFirstCommandFromCommandBuffer();
                }
      ...
      case 'B': // set speed
                { // first, check whether we have four tokens
                  int v0, v1, v2, v3;
                  n_tokens = sscanf( serialCommandBuffer.c_str(), "B %d %d %d %d", &v0, &v1, &v2, &v3 );
                  
                  if (n_tokens == 4) { // ok, all values are there
                    sprintf( print_buffer, "### Set SMS servo speeds %d %d %d %d\n", v0, v1, v2, v3 );
                    Serial.print( print_buffer ); Serial.flush();
      ...
      case 'F': // freeze: set new servo setpoint to current position
                sprintf( print_buffer, "*** Freeze command, pos is %5d\n",
                                joint_position_counts[0] );
      ...
      case 'G': // servo-gripper postion goal: <int> (servo position counts!)
                {
                int tmp[ N_SERVOS ];
                n_tokens = sscanf( serialCommandBuffer.c_str(), "G %d %d %d %d", &tmp[0], &tmp[1], &tmp[2], &tmp[3] );
                if (n_tokens == 4) {
                  setJointPositionGoal( tmp );
                }
                else {
                  sprintf( print_buffer, "### wrong number of tokens %d (req'd 4):\n", n_tokens );
                  Serial.print( print_buffer ); Serial.flush();
                }
                removeFirstCommandFromCommandBuffer();
                }
      ...
      /*
      case 'I': // ignored. Makes no sense for the gripper.
                // thumb abduction by given degrees (positive = towards first finger side
                {
                  int abduction = 0;
                  int tmp[ N_SERVOS ];
                  n_tokens = sscanf( serialCommandBuffer.c_str(), "I %d", &abduction );
                
                  if (n_tokens == 1) {
                    for( int j=0; j < N_JOINTS; j++ ) {
                      tmp[j] = joint_position_goals[j];
                    }
                    // tmp[THJ3] = tmp[THJ3] + abduction;
                    setJointPositionGoal( tmp );
                  }
                  else {
                    sprintf( print_buffer, "### wrong number of tokens %d (req'd 7):\n", n_tokens );
                    Serial.print( print_buffer ); Serial.flush();
                  }
                }
                removeFirstCommandFromCommandBuffer();
      ...
      */
      ...
      case 'K': // upload constant (all LEDs) Neopixel (r,g,b) _K_olor
                Serial.println( "### K: neopixel 'k'olor..." );
                {
                  int red, green, blue;
                  n_tokens = sscanf( serialCommandBuffer.c_str(), "K %d %d %d", &red, &green, &blue );
                  if (n_tokens == 3) {
                    sprintf( print_buffer, "Neopixel color %d %d %d \n", red, green, blue );
                    neopixel_single_color( red, green, blue );
                    Serial.print( print_buffer ); Serial.flush();
                  }
                  else {
                    sprintf( print_buffer, "### wrong number of tokens %d (req'd 3):\n", n_tokens );
                    Serial.print( print_buffer ); Serial.flush();
                  }
                  removeFirstCommandFromCommandBuffer();
                }
      ...
      case 'L': // upload one Neopixel _L_ed color: index r g b 
                Serial.println( "### L: neopixel 'L'ed..." );
                {
                  int led, red, green, blue;
                  n_tokens = sscanf( serialCommandBuffer.c_str(), "L  %d %d %d %d", &led, &red, &green, &blue );
                  if (n_tokens == 4) {
                    sprintf( print_buffer, "Neopixel led %d color %d %d %d \n", led, red, green, blue );
                    neopixel_one_rgb( led, red, green, blue );
                    Serial.print( print_buffer ); Serial.flush();
                  }
                  else {
                    sprintf( print_buffer, "### wrong number of tokens %d (req'd 4):\n", n_tokens );
      ...
      case 'N': // upload one full _N_eopixel pattern: 12 LEDs <rrggbb> each (72 characters)                   
                // N 0 1 255 256 257 258 65536 65537 65792 ... 16711680
                Serial.println( "### L: _N_eopixel pattern..." );
                {
                  int c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11;
                  n_tokens = sscanf( serialCommandBuffer.c_str(), "N %d %d %d %d %d %d %d %d %d %d %d %d", 
                                     &c0, &c1, &c2, &c3,
                                     &c4, &c5, &c6, &c7,
                                     &c8, &c9, &c10, &c11 );
                  if (n_tokens == 12) {
                    sprintf( print_buffer, "Neopixel pattern %d %d %d ... %d %d %d\n",
                             (c0 & 0xff0000) >> 16,
                             (c0 & 0x00ff00) >> 8,
                             (c0 & 0x0000ff) >> 0,
                             (c11 & 0xff0000) >> 16,
                    ...
                    neopixel_one_rgb( 0, (c0 & 0xff0000)>>16, (c0 & 0x00ff00)>>8, (c0 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 1, (c1 & 0xff0000)>>16, (c1 & 0x00ff00)>>8, (c1 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 2, (c2 & 0xff0000)>>16, (c2 & 0x00ff00)>>8, (c2 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 3, (c3 & 0xff0000)>>16, (c3 & 0x00ff00)>>8, (c3 & 0x0000ff)>>0 );

                    neopixel_one_rgb( 4, (c4 & 0xff0000)>>16, (c4 & 0x00ff00)>>8, (c4 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 5, (c5 & 0xff0000)>>16, (c5 & 0x00ff00)>>8, (c5 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 6, (c6 & 0xff0000)>>16, (c6 & 0x00ff00)>>8, (c6 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 7, (c7 & 0xff0000)>>16, (c7 & 0x00ff00)>>8, (c7 & 0x0000ff)>>0 );
                    
                    neopixel_one_rgb( 8, (c8 & 0xff0000)>>16, (c8 & 0x00ff00)>>8, (c8 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 9, (c9 & 0xff0000)>>16, (c9 & 0x00ff00)>>8, (c9 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 10, (c10 & 0xff0000)>>16, (c10 & 0x00ff00)>>8, (c10 & 0x0000ff)>>0 );
                    neopixel_one_rgb( 11, (c11 & 0xff0000)>>16, (c11 & 0x00ff00)>>8, (c11 & 0x0000ff)>>0 );

                    Serial.print( print_buffer ); Serial.flush();
                  }
                  else {
                    sprintf( print_buffer, "### wrong number of tokens %d (req'd 12):\n", n_tokens );
                    Serial.print( print_buffer ); Serial.flush();
                  }
                  removeFirstCommandFromCommandBuffer();
                }
      ...
      case 'P': // "parallel-grasp" mode, move joints to given joint position (counts)
                // 
                {
                  int val = 0;
                  n_tokens = sscanf( serialCommandBuffer.c_str(), "P %d", &val );
                  if (n_tokens == 1) {
                    sprintf( print_buffer, "### Set parallel-grasp delta-pos %d\n", val );
                    Serial.print( print_buffer ); Serial.flush();

                    if ((val > -990) && (val < 990)) { // hardcoded thresholds!!!
                       joint_position_counts[0] = servo_zero_positions[0] + val;
                       joint_position_counts[1] = servo_zero_positions[1] - val;
                       joint_position_counts[2] = servo_zero_positions[2] - val;
                       joint_position_counts[3] = servo_zero_positions[3] + val;
                     
                       setJointPositionGoal( joint_position_counts );
                    }  
                    else Serial.print( "### Set parallel-grasp: delta out of range!!!" );
                  }
                  else {
                    sprintf( print_buffer, "Invalid command '%s'\n", serialCommandBuffer.c_str() );
                    Serial.print( print_buffer ); Serial.flush();
      ...
      case 'T': // servo-hand torque limits: 1 <ints> (servo position counts!)
                {
                int tmp[ N_SERVOS ];
      ...
      case 'm': // print load (torque) (SMS)
                sprintf( print_buffer, "torquem %d\n", sms.ReadLoad( sid ) );
                Serial.print( print_buffer ); Serial.flush();
                removeFirstCommandFromCommandBuffer();
                break;
...
void setup()
{
  ...
  Serial.println( "... Searching SCS/SMS servos..." );
  Serial.println( "... wait until loop completes, then enter serial commands:" );
  usage();
  ...
  // enable the Neopixels: one short rainbow for showing off, then some dark blue...
  //
  init_neopixel_ring();
  neopixel_rainbow_cycle( 20 );
  neopixel_single_color( 0, 0, 3 );

  // detect and initialize the Feetech SCS servos. 
  // Note: different servo types on same bus, same serial port, baudrate default is 1Mbaud
  ...
  for( int id=2; id <= 25; id++ ) { // 253!
    reply = scs.Ping( id );
    if (reply == id) {
      sprintf( print_buffer, "\n\nServo found at ID %3d %2x ping %d\n", id, id, reply );
      Serial.print( print_buffer ); Serial.flush();
  ...
  commandDelay.start( 5000, AsyncDelay::MILLIS );

  Serial.println( "Sleeping for 3000 msec to allow manual shutdown..." );
  delay( 3000 );
...
int last_teach_mode_update_time = 0;
float MFJ1_torque_filtered = 0;
float MFJ1_pos_error_filtered = 0;
float alpha = 0.2;
...
void loop() {
  ...
  if (Serial.available()) {
    handle_command();
    commandDelay.expire(); commandDelay.repeat(); return;
  }
